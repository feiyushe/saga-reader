...
pub trait FeaturesAPI {
    ...
    /// 获得制定订阅包中的订阅信息
    fn get_feeds_by_package(&self, package_id: &str) -> impl std::future::Future<Output=Option<FeedsPackage>>;
    ...
    fn query_by_id(&self, id: i32) -> impl std::future::Future<Output=anyhow::Result<Option<Model>>>;
    fn mark_as_read(&self, id: i32) -> impl std::future::Future<Output=anyhow::Result<()>>;
    fn set_favorite(&self, id: i32, is_favorite: bool) -> impl std::future::Future<Output=anyhow::Result<()>>;

    /// 读取AppConfig
    fn get_app_config(&self) -> impl std::future::Future<Output=anyhow::Result<AppConfig>>;

    /// 覆盖存储AppConfig
    fn set_app_config(&self, app_config: AppConfig) -> impl std::future::Future<Output=anyhow::Result<()>>;
    ...
    fn open_article_external(&self, url: &str) -> impl std::future::Future<Output=anyhow::Result<()>>;

    fn update_article_by_source(&self, article_id: i32, source_text: String) -> impl std::future::Future<Output=anyhow::Result<bool>>;

    fn chat_with_article_assistant(&self, article_id: i32, user_prompt: &str, history: Vec<ConversationMessage>) -> impl std::future::Future<Output=anyhow::Result<String>>;

    fn search_contents_by_keyword(&self, keyword: &str, offset: u64, count: u64) -> impl std::future::Future<Output=anyhow::Result<Vec<Model>>>;
}
