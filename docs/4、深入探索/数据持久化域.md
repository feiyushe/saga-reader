...
```mermaid
graph TD
    A[前端后端模块] --> B[ArticleRecorderService]
    B --> C[Operator]
    C --> D[SQLite数据库]
    C --> E[数据库连接池]
    F[系统集成域] --> G[路径配置]
...
sequenceDiagram
    ...
    participant Store as articles_search_store
    participant Recorder as ArticleRecorderService
    participant Operator as Operator
    participant DB as SQLite

    SearchBar->>Store: "更新filterText为AI"
    Store->>Recorder: "search_contents_by_keyword"
    Recorder->>Operator: "query内容查询"
    Operator->>DB: "SELECT查询"
    DB-->>Operator: "返回匹配文章列表"
    Operator-->>Recorder: "返回结果"
    Recorder-->>Store: "返回文章ID列表"
    Store->>Store: "更新文章列表状态"
    Store->>SearchBar: "触发UI重渲染"
```
