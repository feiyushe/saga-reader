<script lang="ts">
  ...
  let llmFormErrorOllamaModelName = $derived(
    isTextEmpty(llmFormOllamaModelName),
  );
  let llmFormErrorGLMKey = $derived(isTextEmpty(llmFormGLMKey));
  let llmFormErrorPlatformModelPath = $derived(
    isTextEmpty(llmFormPlatformModelPath),
  );

  let llmFormErrorOpenAILikeBaseURI = $derived(
    isTextEmpty(llmFormOpenAILikeBaseURI),
  );
  let llmFormErrorOpenAILikeModelName = $derived(
    isTextEmpty(llmFormOpenAILikeModelName),
  );
  ...
  let llmFormChangedOllama = $derived.by(() => {
    ...
    return (
      llmFormOllamaURI !==
        appConfig.llm.provider_ollama.endpoint.api_base_url ||
  ...
  const switchToLLMOllama = createLLMSwitcherAction(
    ...
    () => {
      ...
      appConfig.llm.provider_ollama.endpoint.api_base_url =
        llmFormOllamaURI || "";
      appConfig.llm.provider_ollama.endpoint.model =
        llmFormOllamaModelName || "";
    },
  );
  ...
  const switchToLLMPlatform = createLLMSwitcherAction(
    ...
    () => {
      if (!appConfig) return;
      appConfig.llm.provider_platform.model_path =
        llmFormPlatformModelPath || "";
    },
  );

  // LLM表单变更保存与还原操作函数
  const saveLLMFormOllama = createSaveLLMFormAction(() => {
    if (!appConfig || llmFormErrorOllamaURI || llmFormErrorOllamaModelName)
      return false;
    appConfig.llm.provider_ollama.endpoint.api_base_url =
      llmFormOllamaURI || "";
...
<div
  ...
  class="cursor-default w-full h-screen overflow-hidden flex flex-col"
>
    ...
    {:else}
      <div class="flex-1 h-full overflow-y-auto pl-8 pr-8">
          ...
          >
            ...
            >
              ...
              <Switch
                name="llm_ollama"
                readOnly
                checked={activedProviderType === "ollama"}
              />
            </div>
            ...
            <p class="mt-2 ml-0.5 type-scale-1 text-surface-400">
              ...
              <span
                ...
                >{$_(
                  "settings.section_llm_provider.provider_ollama_sentence_1",
                )}</span
              >。
            </p>
            {#if activedProviderType === "ollama"}
              ...
              <label
                class={`label ${llmFormErrorOllamaURI ? "text-red-500" : ""}`}
              >
                <span class="label-text"
                  >{$_(
                    "settings.section_llm_provider.provider_ollama_sentence_2",
                  )}</span
                >
                <input
                  class="input p-2"
                  ...
                  placeholder={$_(
                    "settings.section_llm_provider.provider_ollama_sentence_5",
                  )}
                />
              </label>
              <label
                class={`label mt-2 ${llmFormErrorOllamaModelName ? "text-red-500" : ""}`}
              >
                <span class="label-text"
                  >{$_(
                    "settings.section_llm_provider.provider_ollama_sentence_3",
                  )}</span
                >
                <input
                  class="input p-2"
                  ...
                  placeholder={$_(
                    "settings.section_llm_provider.provider_ollama_sentence_4",
                  )}
                />
              </label>

              {#if llmFormChangedOllama}
                {@render LLMGroupSavePanel(
                  saveLLMFormOllama,
          ...
          >
            ...
            >
              ...
              <Switch
                name="llm_glm"
                readOnly
                checked={activedProviderType === "glm"}
              />
            </div>
            <p class="mt-2 ml-0.5 type-scale-1 text-surface-400">
              <span
                >{$_(
                  "settings.section_llm_provider.provider_glm_sentence_1",
                )}</span
              >
              <span
                ...
                class="cursor-pointer text-primary-500"
                >{$_(
                  "settings.section_llm_provider.provider_glm_sentence_2",
                )}</span
              >。
            </p>
            {#if activedProviderType === "glm"}
              <hr class="hr mt-2 mb-2" />
              <label
                class={`label ${llmFormErrorGLMKey ? "text-red-500" : ""}`}
              >
                ...
                <input
                  ...
                  placeholder={$_(
                    "settings.section_llm_provider.provider_glm_sentence_3",
                  )}
                />
              </label>
          ...
          >
            ...
            >
              ...
              <Switch
                name="llm_openai"
                readOnly
                checked={activedProviderType === "openai"}
              />
            </div>
            <p class="mt-2 ml-0.5 type-scale-1 text-surface-400">
              <span
                >{$_(
                  "settings.section_llm_provider.provider_openai_sentence_1",
                )}</span
              >
            </p>
            {#if activedProviderType === "openai"}
              <hr class="hr mt-2 mb-2" />
              <label
                class={`label ${llmFormErrorOpenAILikeBaseURI ? "text-red-500" : ""}`}
              >
                ...
                <input
                  ...
                  placeholder={$_(
                    "settings.section_llm_provider.provider_openai_sentence_2",
                  )}
                />
              </label>
              <label
                class={`mt-2 label ${llmFormErrorOpenAILikeKey ? "text-red-500" : ""}`}
              >
                <span class="label-text">API KEY</span>
                <input
                  class="input p-2"
                  ...
                  placeholder={$_(
                    "settings.section_llm_provider.provider_openai_sentence_3",
                  )}
                />
              </label>
              <label
                class={`mt-2 label ${llmFormErrorOpenAILikeModelName ? "text-red-500" : ""}`}
              >
                ...
                <input
                  ...
                  placeholder={$_(
                    "settings.section_llm_provider.provider_openai_sentence_4",
                  )}
                />
              </label>
        ...
        {@render CheckOption(
          ...
          $_(
            "settings.section_app_behavior.option_scheduled_fetch.description",
          ),
